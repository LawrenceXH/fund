...
class FundSpiders():
    ...
    def getFundCodesFromCsv(self):
        ...
        fund_code = pd.read_csv(filepath_or_buffer=file_path, encoding='gbk', converters = {u'trade_code':str})
    ...
    def getFundInfo(self, fund_code):
        ...
        try:
            print(self.getCurrentTime(), 'getFundInfo:', result['fund_code'], result['fund_name'],
                  result['fund_abbr_name'], result['fund_manager'], result['funder'], result['establish_date'],
                  result['establish_scale'], result['benchmark']
        ...
        for tr in tab.findAll('tr'):
            ...
            if tr.findAll('td'):  # and len((tr.findAll('td')))==7 :
                ...
                try:
                    ...
                    print(self.getCurrentTime(), 'fund_managers_chg:', result['fund_code'], i, result['start_date'],
                          result['end_date'], result['fund_managers'], result['term'], result['return_rate'])
                except  Exception as e:
                ...
                for a in tr.findAll('a'):
                    if a:
                        try:
                            ...
                            print(self.getCurrentTime(), 'getFundManagers3', fund_code, manager['manager_name'],
                                  manager['url'], fund_url, e)

                        try:
                            mySQL.insertData('fund_managers_info', manager)
                            print(self.getCurrentTime(), 'fund_managers_info:', fund_code, manager['manager_name'],
                                  manager['url'], manager['manager_id'])
                        except  Exception as e:
        ...
        for tr in tab.findAll('tr'):
            ...
            if tr.findAll('td') and len((tr.findAll('td'))) == 7:
                ...
                try:
                    ...
                    print(self.getCurrentTime(), 'fund_nav', str(i) + '/' + str(records), result['fund_code'],
                          result['the_date'], result['nav'], result['add_nav'], result['nav_chg_rate'],
                          result['buy_state'], result['sell_state'], result['div_record'])
                except  Exception as e:
                ...
                try:
                    ...
                    print(self.getCurrentTime(), 'fund_nav_currency', str(i) + '/' + str(records), result['fund_code'],
                          result['the_date'], result['profit_per_units'], result['profit_rate'], result['buy_state'],
                          result['sell_state'])
                except  Exception as e:
